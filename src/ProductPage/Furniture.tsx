...
const Furniture = ({ navigation }: { navigation: any }) => {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: "white", width: Dimensions.get('screen').width, height: Dimensions.get('screen').height }}>
            <View style={{ flexDirection: 'row' }}>
                ...
                <Text style={{ top: 20, left: 50, fontSize: 20, color: "black", fontWeight: '800' }}> Furniture </Text>
                <Icon name='ios-options-outline' color={'black'} size={35} style={{ top: 15, left: 170, }} />
            </View>
            <View>
            ...
            {loading ? <ActivityIndicator style={{ justifyContent: "center", alignItems: "center", top: 150 }} size={35} color='black' /> : <View style={{ top: 40, flex: 30 }}>
                {allProduct.length <= 0 ?
                    nothingShow()
                    : <FlatList
